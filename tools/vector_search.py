...
VECTOR_SEARCH_TOOLS = {
    "encyclopedia_search": {
        ...
        "function": {
            ...
            ENDTODO
            Vector search allows you to find relevant information based on the meaning of your query,
            rather than just matching keywords. It uses embeddings of query to understand the context and semantics
            of the text, enabling more accurate and meaningful search results.
            Use this tool for:
            - Searching through encyclopedia data for relevant information
            - Finding answers to questions about the Aetherian universe
            - Retrieving context for specific topics or events
            - Locating information about characters, places, and events in the Aetherian lore
            - Exploring relationships between different entities in the Aetherian universe
...
def _load_embeddings():
    ...
    print(
        f"Loaded {len(_VECTOR_DATABASE_STORE)} embeddings from {VECTOR_DB_CACHE_FILE}"
    )
    return True


def initialize_or_load_vector_db(
    markdown_filepath: str, max_workers: int = 4, force_regenerate: bool = False
) -> Dict[str, Any]:
    """Load from cache or generate new embeddings."""
    if not force_regenerate and _load_embeddings():
        return {
            "status": "loaded_from_cache",
            "total_entries": len(_VECTOR_DATABASE_STORE),
        }
...
def _get_embedding(text: str) -> np.ndarray:
    ...
    if _BEDROCK_CLIENT is None:
        raise RuntimeError(
            "AWS Bedrock client not initialized. Please configure AWS credentials."
        )
    ...
    try:
        ...
        if final_embedding.shape[0] != EMBEDDING_DIMENSION:
            raise ValueError(
                f"Embedding dimension {final_embedding.shape[0]} does not match expected dimension {EMBEDDING_DIMENSION}"
            )
...
def _process_chunk_for_embedding(
    chunk_text: str, markdown_filepath: str, chunk_id: int
) -> Dict[str, Any]:
    """
    ...
    return {"text": chunk_text, "chunk_id": chunk_id, "embedding": embedding_vector}


def initialize_vector_db_from_markdown(
    markdown_filepath: str, max_workers: int = 4
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not chunks:
            return {
                "status": "error",
                "message": "No content chunks found in the file.",
            }

        total_chunks = len(chunks)
        print(
            f"Starting processing of {total_chunks} chunks from {markdown_filepath} using {EMBEDDING_MODEL}..."
        )
        processed_count = 0
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_chunk_data = {
                executor.submit(
                    _process_chunk_for_embedding, chunk, markdown_filepath, i
                ): i
                for i, chunk in enumerate(chunks)
            }

            for i, future in enumerate(
                concurrent.futures.as_completed(future_to_chunk_data)
            ):
                ...
                try:
                    ...
                    if (i + 1) % 1 == 0 or (i + 1) == total_chunks:
                        print(
                            f"Processed and embedded chunk {i + 1}/{total_chunks} (Original index: {chunk_index_in_original_list}) from {markdown_filepath}"
                        )
                except Exception as exc:
                    print(
                        f"Chunk {chunk_index_in_original_list} from {markdown_filepath} generated an exception during processing/embedding: {exc}"
                    )

        print(
            f"Finished processing. Added {processed_count}/{total_chunks} chunks to the in-memory database."
        )
        return {
            "status": "success",
            "chunks_added": processed_count,
            "total_chunks_in_db": len(_VECTOR_DATABASE_STORE),
        }
        ...
        return {
            "status": "error",
            "message": f"Failed to initialize vector DB: {str(e)}",
        }
    ...
    *, message: str, top_k: int = 5
    ...
    if top_k > 25:
        return ToolResult.err(
            "No more than 25 results may be returned for a single query."
        )
    ...
    try:
        ...
        for entry in _VECTOR_DATABASE_STORE:
            ...
            if isinstance(stored_embedding, np.ndarray):
                ...
                print(
                    f"Warning: Skipping entry due to missing or invalid embedding: {entry.get('chunk_id')}"
                )
