...
from objects.prompts.winston import (
    WinstonPlanAnswerPrompt,
    WinstonAnswerWithResultsPrompt,
)
...
class Winston(Model):
    ...
    prompt_answer_with_results: WinstonAnswerWithResultsPrompt = (
        WinstonAnswerWithResultsPrompt()
    )
    ...
    def __init__(
        ...
        initialize_or_load_vector_db(
            "/Users/wylerzahm/Desktop/projects/fc2025-space-agent/objects/datasets/knowledge_base.md"
        )
    ...
    def predict(
        self, messages: List[Dict[str, str]], callback: Optional[Callable] = None
    ) -> Dict[str, Any]:
    ...
    def process(
        self, query: str, callback: Callable[[Dict[str, Any]], None] = None
    ) -> Dict[str, Any]:
        """
            ...
            for iters in range(3):
                ...
                if executed_plan_response and "process" in executed_plan_response:
                    ...
                    if self.finetuned_model:
                        final_response = self._solve_with_results_finetuned(
                            query,
                            executed_plan_response["process"]["execution_summary"],
                        )
                    else:
                        final_response = self._solve_with_results(
                            query,
                            executed_plan_response["process"]["execution_summary"],
                        )
    ...
    def _plan_or_answer(
        self, query: str, execution_results: Vincent.VincentExecuteResult = None
    ) -> Dict[str, Any]:
    ...
    def _execute_plan(
        ...
        execution_result: Vincent.VincentExecuteResult = self.vincent.execute(
        ...
        if execution_result:
            if execution_result.completed:
                ...
                summary_status = (
                    f"Plan execution failed at step {execution_result.failure_step}."
                )
            ...
            for i, output in enumerate(execution_result.outputs):
                ...
                status = (
                    ...
                    if execution_result.completed
                        ...
                        "failed"
                        if i + 1 == execution_result.failure_step
                        else "completed"
                    )
                )
    ...
    def _solve_with_results(
        ...
        # Generate the dynamic system message, ensure system message is always first
        system_message = self.prompt_answer_with_results.system_prompt(
    ...
    def _solve_with_results_finetuned(
        ...
        final_response = {
            "type": "answer",
            "content": {"message": response, "reason": "Finetuned model response"},
        }
        return final_response
